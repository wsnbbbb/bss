...
export default class MyTabs extends React.PureComponent {
  ...
  constructor (props) {
    super(props);
    this.MAXLENGTH = Math.floor(window.innerWidth / 175) - 1; // 最多显示MAXLENGTH个选项卡
    ...
    if (this.props.location.pathname != '/' && this.props.location.pathname != '/home') {
      tabsList = [{
        ...
        code: PATHCONFIG.getPathInfo('path', this.props.location.pathname).code || '',
  ...
  componentWillReceiveProps (nextprops) {
    ...
    let cuurent_tab = PATHCONFIG.getPathInfo('path', nextprops.location.pathname);
    // 不存在 且达到this.MAXLENGTH的最大长度限制
    if (index == -1 && this.tabsLegth == this.MAXLENGTH) {
      tabsList.splice(0, 1);
      tabsList.push({
        pathname: nextprops.location.pathname,
        name: cuurent_tab.name,
        code: cuurent_tab.code,
        search: nextprops.location.search,
      });
    }
    // 不存在且不满this.MAXLENGTH个
    if (index == -1 && this.tabsLegth < this.MAXLENGTH) {
        ...
        <div className="w">
          <div className="tabs">
            <Link to={{
              ...
              <span className={classNames({"tab-name": true, active: '/' == currentPath || '/home' == currentPath})}><DesktopOutlined /> 工作台</span>
            </Link>
          </div>
          {
            _.map(tabsList, (item, index) => <div key={item.pathname} className="tabs">
              <Link
                to={{
                ...
                <span className={classNames({"tab-name": true, active: item.pathname == currentPath})} onClick={() => {this.change(index, item);}}>{`${item.name}(${item.code})`}</span>
              </Link>
