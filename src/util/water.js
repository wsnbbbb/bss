...
// let userName = User && User.getInfo().name || '';
// watermark({watermark_txt: `${userName} ${moment().format('L')}`});
export function watermark (settings) {
  ...
  for (let i = 0; i < defaultSettings.watermark_rows; i++) {
    y = defaultSettings.watermark_y + (defaultSettings.watermark_y_space + defaultSettings.watermark_height) * i + defaultSettings.index * 10;
...
export function clearWatermark () {
  let mask_div = document.body.getElementsByClassName('mask_div');
  let len = mask_div.length;
  for (let i = 0; i < len; i++) {
    mask_div[i].remove();
  }
}
