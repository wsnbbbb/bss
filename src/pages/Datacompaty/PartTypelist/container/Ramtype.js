...
class Ram extends React.Component {
    ...
    onGetMListData (param = {}) {
      ...
      http.get(`${BSS_ADMIN_URL}/api/old/parts/memory/list`, { params: tools.clearEmpty(params) })
        .then((res) => {
          res = res.data;
    ...
    onModelOk = (value) => {
      this.setState({ loading: true });
      http
        .post(`${BSS_ADMIN_URL}/api/old/parts/memory/${this.props.record.id}/add`, value)
        .then((res) => {
          res = res.data;
          if (res.code === 20000) {
    ...
    // 构建字段
    makeColumns () {
      const {mem_spec, memory_type, mem_verify} = SYS_DICT_SERVERPART;
      const columns = [
        {
          title: "内存型号名称",
          dataIndex: "memName",
        ...
        },
        {
          ...
          dataIndex: "memSpec",
          key: "memSpec",
          render: (text, record) => mem_spec[text],
        },
        {
          title: "内存类型",
          dataIndex: "memType",
          key: "memType",
          render: (text, record) => memory_type[text],
        },
        {
          title: "内存校验",
          dataIndex: "memVerify",
          key: "memVerify",
          render: (text, record) => mem_verify[text],
        },
        {
          title: "备注",
          dataIndex: "remark",
    ...
    render () {
      ...
      let { type, } = this.props;
      // const {mem_verify}=SYS_DICT_DATACOMPATY
      const {mem_spec, memory_type} = SYS_DICT_SERVERPART;
      const pagination = {
        current: pageNum,
        total: total,
        size: page_size,
