...
class Ram extends React.Component {
    ...
    onGetMListData (param = {}) {
      ...
      http.get(`${BSS_ADMIN_URL}/api/old/parts/disk/list`, { params: tools.clearEmpty(params) })
        .then((res) => {
          res = res.data;
    ...
    onModelOk = (value) => {
      this.setState({ loading: true });
      http
        .post(`${BSS_ADMIN_URL}/api/old/parts/disk/${this.props.record.id}/add`, value)
        .then((res) => {
          res = res.data;
          if (res.code === 20000) {
    ...
    // 构建字段
    makeColumns () {
      const {disk_type, disk_interface_type, disk_short, server_disk_spec} = SYS_DICT_SERVERPART;
      const columns = [
        {
          title: "硬盘型号名称",
          dataIndex: "diskName",
