...
export default class list extends React.Component {
  ...
  onGetListData (values) {
    ...
    http.get(`${BSS_ADMIN_URL}/api/customer/salePersonnel`, {
      params: params
    })
      .then((res) => {
        // console.log(res)
...
*/
  ...
  onModalOk (customerRelation) {
    if (this.state.operateType == 'add') {
      ...
      http.post(`${BSS_ADMIN_URL}/api/customer/salePersonnel/add`, param)
        .then((res) => {
          res = res.data;
          console.log(res);
          if (tools.hasStatusOk(res)) {
  ...
  onDel (id) {
    ...
    http.delete(`${BSS_ADMIN_URL}/api/customer/salePersonnel/${id}/delete`)
      .then((res) => {
        res = res.data;
        console.log(res);
        if (res.code === 20000) {
    ...
    const columns = [
      {
        ...
        dataIndex: 'saleType',
        key: 'saleType',
        render: (text, record) => customer_sale_type[text]
      },
      {
        title: '人员',
        dataIndex: 'salesName',
      ...
      },
      {
        ...
        render: (text, record) => {
          ...
          controls.push(
            ...
            >
              {User.hasPermission('salePersonnel-view') &&
          ...
          controls.push(
            ...
            >
              {User.hasPermission('salePersonnel-update') &&
          ...
          controls.push(
            ...
            >
              <span className="control-btn red">
                {User.hasPermission('salePersonnel-delete') &&
    ...
    return (
      <main className="mian">
        <div className="g-search">
          <Form ref={this.searchFormRef} name="searchbox" layout="inline" onFinish={(values) => {this.onSearchfrom(values);}}>
            <Form.Item name="saleType">
              <Select
                placeholder="请选择销售类型" allowClear
                style={{ width: 170 }}>
                {_.map(customer_sale_type, (value, key) => <Option value={parseInt(key)} key={key}>{value}</Option>)}
              </Select>
            </Form.Item>
            <Form.Item name="salesName" >
              <Input placeholder="请输入人员" maxLength="30" allowClear />
            </Form.Item>
        ...
        <div className="g-operate">
          {User.hasPermission('salePersonnel-add') && <Button size="middle" className="actions-btn" onClick={() => this.onModalShow("add")}>增加</Button>}
        </div>
        {/* 数据展示 */}
