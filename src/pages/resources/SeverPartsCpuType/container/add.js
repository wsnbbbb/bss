...
class RoleAdminContainer extends React.Component {
  ...
  render () {
    const { operateType, nowData, cpu_interface_type } = this.props;
    ...
    return (
      <div>
        <Modal
          title={
            { add: "CPU型号添加", up: "CPU型号修改", see: "查看" }[operateType]
          }
          maskClosable={false}
          width="50%"
          destroyOnClose
              ...
              {/* <Col xs={24} sm={24} md={24} lg={12} xl={12}>
                <FormItem
                  ...
                  rules={[{ required: true, message: "必填" },
                    { pattern: regExpConfig.num, message: "请输入正整数" },
              ...
              <Col xs={24} sm={24} md={24} lg={12} xl={12}>
                <FormItem
                  ...
                  rules={[
                    { required: true, message: "必填" },
                    { pattern: regExpConfig.num, message: "请输入正整数" },
                    { type: 'number', max: 999, message: "最多输入3位字符" },
                  ]}
                >
                  <InputNumber
                    placeholder="请输入CPU线程数"
                    disabled={operateType === "see"}
                  />
                </FormItem>
              </Col>
