...
export default class RoleAdminContainer extends React.Component {
  ...
  makeColumns () {
    const columns = [
...

      {
        ...
        dataIndex: "memSize",
        key: "memSize",
        render: (text, record) => <Tag color="#2db7f5">{(record && record.memSize)
        }</Tag>
        ,
      },
      {
      ...
      },
      {
        ...
        dataIndex: "memVerify",
        key: "memVerify",
        render: (text, record) => SYS_DICT_SERVERPART.mem_verify[text],
      },
      {
        title: "备注",
        dataIndex: "remark",
  ...
  onModalShow (type, data) {
    ...
    this.setState({
      ...
      nowData: {
        ...
        memHz: data.memHz,
        memBrand: data.memBrand,
        memModel: data.memModel,
        memSpec: data.memSpec,
        memType: data.memType,
        memVerify: data.memVerify,
        remark: data.remark,
      },
  ...
  onDel (id) {
    ...
    http
      .delete(`${BSS_ADMIN_URL}/api/product/memorymodel/delete`, {
        data: params,
      })
      .then((res) => {
        res = res.data;
  ...
  onModalOk = (val) => {
    ...
    http
      .post(`${BSS_ADMIN_URL}/api/product/memorymodel/add`, tools.clearEmpty(val))
      .then((res) => {
        res = res.data;
        if (res.code === 20000) {
          this.onClose();
  ...
  edit = (id, val) => {
    ...
    val.memName = '';
    http
      .put(`${BSS_ADMIN_URL}/api/product/memorymodel/${id}/update`, tools.clearNull(val))
      .then((res) => {
        res = res.data;
        if (res.code === 20000) {
          this.onClose();
    ...
    return (
      <main className="mian">
        ...
        <div className="c-operate">
          {u1.includes('memorymodel-add') && <Button
            size="middle"
            onClick={() => {
          ...
          {u1.includes('memorymodel-del') && <Button size="middle" onClick={this.showDeleteConfirm}  className="actions-btn">
            批量删除
          </Button>}
        </div>
        {/* 数据展示 */}
