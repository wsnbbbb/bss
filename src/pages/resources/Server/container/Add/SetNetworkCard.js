...
class SetNetworkCard extends React.Component {
  ...
  netCard = (filedName, index, value) => {
    ...
    if (filedName == 'speed' || filedName == 'interfaceType') {
      ...
      this.props.setSolts({
        ipmiIndex: this.state.ipmiIndex,
        lists: lists
      });
      return;
    }
  ...
  },
  {
    ...
    render: (text, record, index) => {
      ...
      // 如果是ipmi
      if (listIndex == this.state.ipmiIndex) {
        return 'ipmi';
      }
      // 非ipmi的按照顺序生成：网卡+序号
      if (listIndex > this.state.ipmiIndex) {
        return `NIC${listIndex  - 1}`;
      }
      if (listIndex < this.state.ipmiIndex) {
        return `NIC${listIndex}`;
      }
    }
  },
  {
    ...
    dataIndex: 'ipmi',
    width: 50,
    render: (text, record, index) => <Select value={(index + 1  == this.state.ipmiIndex) ? 1 : 0}
  ...
  },
  {
    ...
    render: (text, record, index) => <div className="my-input">
      <DeviceRadio
        ...
        <Input style={{width: 320}} placeholder="请选择网络设备" value={record.networkDevice.deviceName || ""}></Input>
      </DeviceRadio>
    </div>
  },
  {
    ...
    render: (text, record, index) => <React.Fragment>
      <PortRadio
        title="选择端口"
        disabled={record.networkDevice.id == undefined}
        disabledTip="请选择交换机！"
        allowClear
        fetchPath={`${BSS_ADMIN_URL}/api/product/network/devices/port?networkDeviceId=${record.networkDevice.id}`}
        onSelect={(id, row) => {this.netCard('devicePort', index + 1, row);}}>
        <Input style={{width: 100}} placeholder="请选择端口" value={record.devicePort.port || ""}></Input>
      </PortRadio>
    </React.Fragment>
