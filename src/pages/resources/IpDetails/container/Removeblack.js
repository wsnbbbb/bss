...
@observer
class Removeblack extends React.Component {
  static propTypes = {
    location: P.any,
    history: P.any,
    root: P.any, // 全局资源
  ...
  render () {
    const { removeBlank_status } = SYS_DICT_IP;
    return (
      <React.Fragment>
            ...
            <Col xs={24} sm={24} md={24} lg={24} xl={24}>
              <Form.Item name="specialStatus" rules={[{ required: true, message: '请选择特殊状态' }]} label="特殊状态"
                ...
                <Select placeholder="请选择特殊状态">
                  {
                    _.map(removeBlank_status, (item, key) => <Option value={parseInt(key)} key={key} > {item} </Option>)
                  }
                </Select>
              </Form.Item>
            </Col>
