...
class PortManage extends React.Component {
  static propTypes = {
    ...
    deviceId: P.string,
    title: P.string,
    onSelect: P.func, // 只关闭弹窗
    hardParam: P.any, // deviceType 和 deviceModelId 设备类型和设备型号必填，查询端口模板作为初始数据。
    onOk: P.func, // 弹框确认
  ...
  getDevicePorts () {
    ...
    http.get(`${BSS_ADMIN_URL}/api/product/network/devices/port`, {
      params: {networkDeviceId: this.props.deviceId}
    })
      .then((res) => {
        res = res.data;
  ...
  onOk = () => {
    ...
    http.post(`${BSS_ADMIN_URL}/api/product/network/devices/port/alladd`, lists)
      .then((res) => {
        res = res.data;
        if (tools.hasStatusOk(res)) {
          this.props.onSelect(this.portkeys.length);
          this.setState({
            modalLoading: false,
            showModal: false
          });
  ...
  onOkEdit = (val) => {
    ...
    if (this.editType == 'one') {
      let index = _.findIndex(lists, (list) => this.nowData.port == list.port);
      lists[index] = {
        ...this.nowData,
        ...val,
      };
