...
export default class List extends React.Component {
  ...
  componentDidMount () {
    ...
    console.log(this.props.activeKey, this.props.houseId);
  }
  ...
  onGetListData = (values) => {
    ...
    http.get(`${BSS_ADMIN_URL}/api/goods/cpuprice`, {
  ...
  onModalShow () {
    ...
    // 获取已设置机房列表
    http.get(`${BSS_ADMIN_URL}/api/goods/cpuprice/2/price_house`)
      .then((res) => {
        res = res.data;
        if (tools.hasStatusOk(res)) {
  ...
  onModalOk = () => {
    let param = {};
    let url = `${BSS_ADMIN_URL}/api/goods/cpuprice`;
  ...
  render () {
    ...
    const {coreNums, models} =  this.props;
    return (
      <main className="mian">
        {/* 搜索 */}
          ...
          <Form ref={this.searchFormRef} name="searchbox" layout="inline" onFinish={(values) => {this.onSearch(values);}}>
            <Form.Item name="cpuModel">
              <Select
                ...
                {
                  _.map(models, (item) =>  <Select.Option value={item} key={item}>{item}</Select.Option>)
                }
              </Select>
            </Form.Item>
            <Form.Item name="coreNum">
              <Select
                style={{width: 200}}
                placeholder="核心数" allowClear>
                {
                  _.map(coreNums, (item) =>  <Select.Option value={item} key={item}>{item}</Select.Option>)
                }
              </Select>
            </Form.Item>
            <Form.Item>
        ...
        <div className="g-table">
          <Spin spinning={loading}>
            <PriceTable data={lists} postData={{houseId: this.props.houseId}} postUrl={`${BSS_ADMIN_URL}/api/goods/cpuprice`} columns={columns}
              ...
              <div style={{marginTop: 20}}>
                <Radio value={2}>
                  ...
                  <div className="fileds">
                    <div className="filed">
                      <span className="filed-lable">型号</span>
                      <Select style={{ width: 400 }} onChange={(val) => {this.onChangeConfig('cpuModel', val);}}>
                    ...
                    <div className="filed">
                      <span className="filed-lable">核数</span>
                      <Select style={{ width: 400 }} onChange={(val) => {this.onChangeConfig('coreNum', val);}}>
                        <Select.Option value={null} key={null}>任意</Select.Option>
                        {
                          _.map(coreNums, (item) =>  <Select.Option value={item} key={item}>{item}</Select.Option>)
                        }
