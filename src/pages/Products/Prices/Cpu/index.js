...
class CpuPrice extends React.Component {
  ...
  fetchCpuCodeNum () {
    ...
    http.get(`${BSS_ADMIN_URL}/api/product/cpumodel/cpucode`)
      .then((res) => {
        res = res.data;
  ...
  fetchCpuModel () {
    ...
    http.get(`${BSS_ADMIN_URL}/api/product/cpumodel/modelname`)
      .then((res) => {
        res = res.data;
        if (tools.hasStatusOk(res)) {
          this.setState({
  ...
  render () {
    const { mode, activeKey, models, coreNums } = this.state;
        ...
        <Tabs defaultActiveKey={activeKey1} tabPosition={mode} onChange={this.setActiveKey} activeKey={activeKey1}>
          {_.map(houseList, (house) => (
            <TabPane tab={house.fullName} key={house.id}>
              <List houseId={house.id} activeKey={activeKey1}  models={models} coreNums={coreNums}></List>
            </TabPane>
          ))}
        </Tabs>
      </div>
