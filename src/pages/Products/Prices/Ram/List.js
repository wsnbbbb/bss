...
import {
  Form, Button, Select, Radio, Pagination, Modal, InputNumber, Spin,
} from 'antd';
...
export default class List extends React.Component {
  ...
  onGetListData = (values) => {
    ...
    http.get(`${BSS_ADMIN_URL}/api/goods/memoryprice`, {
  ...
  onModalShow () {
    ...
    // 获取已设置机房列表
    http.get(`${BSS_ADMIN_URL}/api/goods/cpuprice/3/price_house`)
      .then((res) => {
        res = res.data;
        if (tools.hasStatusOk(res)) {
  ...
  onModalOk = () => {
    let param = {};
    let url = `${BSS_ADMIN_URL}/api/goods/memoryprice`;
    ...
    http.post(`${url}`, tools.clearEmpty(param))
      .then((res) => {
        res = res.data;
        if (tools.hasStatusOk(res)) {
          this.onGetListData();
  ...
  render () {
    ...
    },
    {
      ...
      dataIndex: "memType",
      key: "memType",
      render: (text) => SYS_DICT_SERVERPART.memory_type[text] || ''
    },
    {
      title: "容量（G）",
      dataIndex: "memSize",
      ...
      <main className="mian">
          ...
          <Form ref={this.searchFormRef} name="searchbox" layout="inline" onFinish={(values) => {this.onSearch(values);}}>
            <Form.Item name="memType">
              <Select
                ...
                {
                  _.map(SYS_DICT_SERVERPART.memory_type, (item, key) =>  <Select.Option value={parseInt(key)} key={key}>{item}</Select.Option>)
                }
              </Select>
            </Form.Item>
            <Form.Item name="memSize">
              <Select
                ...
                {
                  _.map(morySize, (item) =>  <Select.Option value={item} key={item}>{item}</Select.Option>)
                }
              </Select>
            </Form.Item>
            <Form.Item>
        ...
        <div className="g-table">
          <Spin spinning={loading}>
            <PriceTable data={lists} postData={{houseId: this.props.houseId}} postUrl={`${BSS_ADMIN_URL}/api/goods/memoryprice`} columns={columns}
                ...
                <Radio value={2}>
                  ...
                  <div className="fileds">
                    <div className="filed">
                      ...
                      <Select
                        style={{width: 200}} onChange={(val) => {this.onChangeConfig('memType', val);}}
                        placeholder="类型" allowClear>
                        <Select.Option value={null} key={null}>任意</Select.Option>
                        {
                          _.map(SYS_DICT_SERVERPART.memory_type, (item, key) =>  <Select.Option value={parseInt(key)} key={item}>{item}</Select.Option>)
                        }
                      </Select>
                    </div>
                    <div className="filed">
                      <span className="filed-lable">容量</span>
                      <Select style={{ width: 400 }} onChange={(val) => {this.onChangeConfig('memSize', val);}}>
                        <Select.Option value={null} key={null}>任意</Select.Option>
                        {
                          _.map(morySize, (item) =>  <Select.Option value={item} key={item}>{item}</Select.Option>)
                        }
