...
export default class List extends React.Component {
  ...
  onGetListData = (values) => {
    ...
    // http.get(`${BSS_ADMIN_URL}/api/goods/raidprice`, {
    // http.get(`${BSS_ADMIN_URL}/api/goods/raidprice`, {
    http.get(`${BSS_ADMIN_URL}/api/goods/raidprice`, {
  ...
  onModalShow () {
    ...
    // 获取已设置机房列表
    http.get(`${BSS_ADMIN_URL}/api/goods/cpuprice/5/price_house`)
      .then((res) => {
        res = res.data;
        if (tools.hasStatusOk(res)) {
  ...
  onModalOk = () => {
    let param = {};
    let url = `${BSS_ADMIN_URL}/api/goods/raidprice`;
  ...
  render () {
    ...
    },
    {
      ...
      dataIndex: "raidType",
      key: "raidType",
      render: (text, records) => SYS_DICT_SERVERPART.raid_type[text]
    },
    {
      title: "缓存",
      dataIndex: "raidCache",
    ...
    const {raidcache} =  this.props;
    return (
      <main className="mian">
        {/* 搜索 */}
        <div className="g-search">
          <Form ref={this.searchFormRef} name="searchbox" layout="inline" onFinish={(values) => {this.onSearch(values);}}>
            <Form.Item name="raidType">
              <Select
                style={{width: 200}}
                placeholder="型号" allowClear>
                {
                  _.map(SYS_DICT_SERVERPART.raid_type, (item, key) =>  <Select.Option value={key} key={key}>{item}</Select.Option>)
                }
              </Select>
            </Form.Item>
            <Form.Item name="raidcache">
              <Select
                style={{width: 200}}
                placeholder="缓存" allowClear>
                {
                  _.map(raidcache, (item) =>  <Select.Option value={item} key={item}>{item}</Select.Option>)
                }
              </Select>
            </Form.Item>
            <Form.Item>
        ...
        <div className="g-table">
          <Spin spinning={loading}>
            <PriceTable data={lists} postData={{houseId: this.props.houseId}} postUrl={`${BSS_ADMIN_URL}/api/goods/raidprice`} columns={columns}
              ...
              <div style={{marginTop: 20}}>
                <Radio value={2}>
                  ...
                  <div className="fileds">
                    <div className="filed">
                      <span className="filed-lable">类型</span>
                      <Select style={{ width: 400 }} onChange={(val) => {this.onChangeConfig('raidType', val);}}>
                    ...
                    <div className="filed">
                      <span className="filed-lable">缓存</span>
                      <Select style={{ width: 400 }} onChange={(val) => {this.onChangeConfig('raidcache', val);}}>
                        <Select.Option value={null} key={null}>任意</Select.Option>
                        {
                          _.map(raidcache, (item) =>  <Select.Option value={item} key={item}>{item}</Select.Option>)
                        }
