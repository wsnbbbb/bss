...
const { TabPane } = Tabs;
class MinResRaid extends React.PureComponent {
  static propTypes = {
    location: P.any,
    history: P.any,
    onSave: P.func, // 保存配置
  ...
  constructor (props) {
    ...
    if (this.props.defaultValue) {
      ...
      selectedParams['raidType'] = this.props.defaultValue.raidType && this.props.defaultValue.raidType.values || [];
      selectedParams['raidCache'] = this.props.defaultValue.raidcache && this.props.defaultValue.raidcache.values || [];
    }
    this.state = {
  ...
  fetchConfigParams () {
    ...
    http.get(`${BSS_ADMIN_URL}/api/goods/raidprice/raid_uniq`)
      .then((res) => {
        res = res.data;
    ...
    return (
      <div className="min-res">
        <div className="min-res-head">
          <span className="min-res-title">+ RAID卡 </span>
        ...
        >
          ...
          <div className="min-res-content">
            ...
            >
              <Select mode="multiple" placeholder="请选择" style={{width: 600}} disabled={editable} allowClear
                onChange={(val, options) => {this.onChangeOption('raidType', options);}}>
                {
                  _.map(SYS_DICT_SERVERPART.raid_type, (value, key) =>  <Select.Option value={parseInt(key)} key={key}>{value}</Select.Option>)
                }
              </Select>
            </Form.Item>
            <Form.Item
              name={['raidType', "defaultValue"]}
              label="默认值"
            ...
            >
              <Select placeholder="默认值" style={{width: 300}} disabled={editable} allowClear>
                {
                  _.map(selectedParams.raidType, (options) =>  <Select.Option value={options.value} key={options.value}>{options.children}</Select.Option>)
                }
              </Select>
            </Form.Item>
            <Form.Item
              name={['raidType', "userSee"]}
              valuePropName="checked"
              style={style}>
          ...
          <div className="min-res-content">
            <Form.Item
              name={['raidCache', "limit"]}
              valuePropName="checked"
              style={style}>
              <Checkbox disabled={editable}>缓存</Checkbox>
            </Form.Item>
            <Form.Item
              name={['raidCache', "values"]}
              label="可选值"
              // rules={[{required: true, message: '请选择', type: 'array'}]}
              style={style2}
            >
              <Select mode="multiple" placeholder="请选择" style={{width: 400}} disabled={editable} allowClear
                onChange={(val, options) => {this.onChangeOption('raidCache', options);}}>
                {
                  _.map(conifgParams.raidcache_list, (value) =>  <Select.Option value={value} key={value}>{value}</Select.Option>)
                }
              </Select>
            </Form.Item>
            <Form.Item
              name={['raidCache', "defaultValue"]}
              label="默认值"
              // rules={[{required: true, message: '请选择' }]}
              style={style1}
            >
              <Select style={{width: 300}} disabled={editable} allowClear>
                {
                  _.map(selectedParams.raidCache, (options) =>  <Select.Option value={options.value} key={options.value}>{options.children}</Select.Option>)
                }
              </Select>
            </Form.Item>
            <Form.Item
              name={['raidCache', "userSee"]}
              valuePropName="checked"
              style={style}>
              <Checkbox disabled={editable}>用户可见</Checkbox>
            </Form.Item>
