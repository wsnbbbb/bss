...
// 登录
const onLogin = (p) => {
  const u = users.find((item) => item.username === p.username);
  if (!u) {
...
// 获取所有菜单
const getMenus = (p) => ({ status: 200, data: menus, message: "success" });
// 获取菜单（根据ID）
...
// 查询角色（所有）
const getAllRoles = (p) => ({ status: 200, data: roles, message: "success" });
// 查询角色（通过角色ID）
const getRoleById = (p) => {
  // const p = JSON.parse(request.body);
  let res = [];
...
const findAllPowerByRoleId = (p) => {
  ...
  if (t) {
    const res = t.powers.map((item, index) => {
      ...
      const _powers = item.powers.map((v) => powers.find((p) => p.id === v));
...
const setPowersByRoleId = (p) => {
  ...
  if (oldIndex !== -1) {
    ...
    pow.filter((item) => {
      const parr = p.powers.filter((v) => powers.findIndex((pv) => pv.menu === item.menuId) > -1);
      item.powers = parr;
    });
 ...
 * **/
// 登录请求
Mock.mock('mockapi/login', (params) => onLogin(params));
// 获取消息数据
Mock.mock('mockapi/getnews', () => ({status: 200, data: msg, total: msg.notice.length + msg.message.length + msg.work.length, message: 'success'}));
// 删除消息数据
Mock.mock('mockapi/clearnews', (params) => clearNews(params));
// 获取消息总数
Mock.mock('mockapi/getnewstotal', () => ({ status: 200, data: msg.notice.length + msg.message.length + msg.work.length, message: 'success' }));
// 获取所有菜单
Mock.mock('mockapi/getmenus', (params) => getMenus(params));
// 获取菜单（根据ID）
Mock.mock('mockapi/getMenusById', (params) => getMenusById(params));
// 添加菜单
Mock.mock('mockapi/addmenu', (params) => addMenu(params));
// 修改菜单
Mock.mock('mockapi/upmenu', (params) => upMenu(params));
// 删除菜单
Mock.mock('mockapi/delmenu', (params) => delMenu(params));
// 根据菜单ID查询其下权限
Mock.mock('mockapi/getpowerbymenuid', (params) => getPowerByMenuId(params));
// 根据权限ID查询对应的权限
Mock.mock('mockapi/getPowerById', (params) => getPowerById(params));
// 添加权限
Mock.mock('mockapi/addpower', (params) => addPower(params));
// 修改权限
Mock.mock('mockapi/uppower', (params) => upPower(params));
// 删除权限
Mock.mock('mockapi/delpower', (params) => delPower(params));
// 查询角色（分页）
Mock.mock('mockapi/getroles', (params) => getRoles(params));
// 查询角色（所有）
Mock.mock('mockapi/getAllRoles', (params) => getAllRoles(params));
// 查询角色（通过角色ID）
Mock.mock('mockapi/getRoleById', (params) => getRoleById(params));
// 添加角色
Mock.mock('mockapi/addrole', (params) => addRole(params));
// 修改角色
Mock.mock('mockapi/uprole', (params) => upRole(params));
// 删除角色
Mock.mock('mockapi/delrole', (params) => delRole(params));
// 根据角色ID查询该角色所拥有的菜单和权限详细信息
Mock.mock('mockapi/findAllPowerByRoleId', (params) => findAllPowerByRoleId(params));
// 获取所有的菜单及权限数据 - 为了构建PowerTree组件
Mock.mock('mockapi/getAllPowers', (params) => getAllPowers(params));
// 给指定角色分配菜单和权限
Mock.mock('mockapi/setPowersByRoleId', (params) => setPowersByRoleId(params));
// 条件分页查询用户列表
Mock.mock('mockapi/getUserList', (params) => getUserList(params));
// 添加用户
Mock.mock('mockapi/addUser', (params) => addUser(params));
// 修改用户
Mock.mock('mockapi/upUser', (params) => upUser(params));
// 删除用户
Mock.mock('mockapi/delUser', (params) => delUser(params));
// 给用户分配角色
Mock.mock('mockapi/setUserRoles', (params) => setUserRoles(params));
